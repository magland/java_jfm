...
public class SSTimeSeriesWidget extends VBox {
	...
	public SSTimeSeriesWidget() {	
		...
		SP_main.setDividerPositions(0.7);
	...
	public void addDataArray(Mda X,Mda label_data) {
		...
		VVmag.setData(X);
		VVmag.setLabelData(label_data);
		VVmag.setLabelData(label_data);
		VVmag.setXRange(0, 200);
		VV.setData(X); //important to do this after so that we get the signal to the magnified view
		VV.setLabelData(label_data);
		
		update_info();
	}
	public void setLabelDataArray(int ind,Mda X) {
		m_views.get(ind).setLabelDataArray(X);
		m_magnified_views.get(ind).setLabelDataArray(X);
	}
	...
	void setup_new_views(SSTimeSeriesView VV,SSTimeSeriesView VVmag) {
		...
		if (m_views.size()>1) {
			...
			for (int i=0; i<m_views.size()-1; i++) {
				double offset=0.1;
				m_SP_left.setDividerPosition(i,(i+1+offset)*1.0/(m_views.size()));
				m_SP_right.setDividerPosition(i,(i+1+offset)*1.0/(m_views.size()));
			}
		}
...
//		});
		...
		if (m_views.size()==0) {
			VV.setChannelColors(view_colors);
			VV.setChannelColors(view_colors);
		}
...
class SSTimeSeriesView extends StackPane {
	...
	public void setLabelDataArray(Mda X) {
		m_label_data=X;
	}
	public void setLabelData(Mda LD) {
		m_label_data=LD;
	}
	...
	void move_current_x_coord(int dx) {
		...
		if (dx<0) {
			...
			if (m_label_data!=null) {
				boolean done=false;
				while ((!done)&&(new_x-1>=0)) {
					boolean all_zeros=true;
					for (int ch=0; ch<m_label_data.size(0); ch++) {
						if (m_label_data.value(ch,new_x)!=0) {
							all_zeros=false;
						}
					}
					if (all_zeros) {
