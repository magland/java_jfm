...
public:
	...
	void construct() {
		m_size=(int *)malloc(sizeof(int)*MDA_MAX_DIMS);
		for (int i=0; i<MDA_MAX_DIMS; i++) m_size[i]=1;
		
		m_data_real=(float *)malloc(sizeof(float)*1);
		m_data_real[0]=0;
	...
	float write_float(FILE *outf,float val);
	void write_short(FILE *outf,short val);	
	void write_unsigned_short(FILE *outf,unsigned short val);
	unsigned char write_unsigned_char(FILE *outf,unsigned char val);
};
...
void Mda::allocate(int num_dims,int *size) {
	...
	if (NN > 0) {
		free(d->m_data_real);
		d->m_data_real = (float *)malloc(sizeof(float)*NN);
	}
}
...
float Mda::value1(int i) const {
	return d->m_data_real[i];
}
float Mda::value(int i1,int i2,int i3,int i4) const {
...
float Mda::value(int num_dims,int *ind) const {
	int tmp=0;
	int factor=1;
	for (int i=0; i<num_dims; i++) {
...
void Mda::setValue1(float val,int i) {
	d->m_data_real[i]=val;
}
...
void Mda::setValue(float val,int num_dims,int *ind) {
	int tmp=0;
	int factor=1;
	for (int i=0; i<num_dims; i++) {
...
Mda Mda::getDataXY(int num_inds,int *inds) const {
	Mda ret;
	ret.allocate(N1(), N2());
	int *inds0=(int *)malloc(sizeof(int)*(num_inds+2));
...
Mda Mda::transpose() const {
	Mda ret;
	int *size=(int *)malloc(sizeof(int)*MDA_MAX_DIMS);
...
float MdaPrivate::write_float(FILE *outf,float val) {
	fwrite(&val,4,1,outf);
}
...
unsigned char MdaPrivate::write_unsigned_char(FILE *outf,unsigned char val) {
	fwrite(&val,1,1,outf);
}
