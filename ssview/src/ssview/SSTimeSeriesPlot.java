		...
		if (m_data.size(1)>1) {
			for (int ch = 0; ch < M; ch++) {
				...
				for (int ii = xrange_min; ii <= xrange_max; ii++) {
					...
					yvals.setValue1(val, ii - xrange_min);
				}
			...
			for (int ch=0; ch<m_num_channels; ch++) {
				...
				while (ii<TS.length) {
					int kk=ii+1;
					while ((kk<TS.length)&&(TS[kk]==TS[ii]+kk-ii)) kk++;
					xvals_count+=kk-ii+2;
					ii=kk;
				}
				Mda xvals=new Mda(); xvals.allocate(1,xvals_count);
				Mda yvals=new Mda(); yvals.allocate(1,xvals_count);
				int xvals_ind=0;
				ii=0;
				while (ii<TS.length) {
					int kk=ii+1;
					...
					yvals.setValue(0, 0,xvals_ind); xvals_ind++;
					for (int jj=ii; jj<kk; jj++) {
						xvals.setValue(TS[jj], 0,xvals_ind);
						yvals.setValue(1, 0,xvals_ind); xvals_ind++;
					}
